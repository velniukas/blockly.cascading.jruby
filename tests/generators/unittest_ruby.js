/**
 * Visual Blocks Language
 *
 * Copyright 2012 Google Inc.
 * http://blockly.googlecode.com/
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

/**
 * @fileoverview Generating Ruby for unit test blocks.
 * @author fraser@google.com (Neil Fraser)
 */
'use strict';

Blockly.Ruby = Blockly.Generator.get('Ruby');

Blockly.Ruby.unittest_main = function() {
  // Container for unit tests.
  var resultsVar = Blockly.Ruby.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  if (!Blockly.Ruby.definitions_['unittest_report']) {
    var functionName = Blockly.Ruby.variableDB_.getDistinctName(
        'testReport', Blockly.Generator.NAME_TYPE);
    Blockly.Ruby.unittest_main.report = functionName;
    var func = [];
    func.push('def ' + functionName + '():');
    func.push('  # Create test report.');
    func.push('  report = []');
    func.push('  summary = []');
    func.push('  fails = 0');
    func.push('  for (success, title, log) in ' + resultsVar + ':');
    func.push('    if success:');
    func.push('      summary.append(".")');
    func.push('    else:');
    func.push('      summary.append("F")');
    func.push('      fails += 1');
    func.push('      report.append("")');
    func.push('      report.append("FAIL: " + title)');
    func.push('      report.append(log)');
    func.push('  report.insert(0, "".join(summary))');
    func.push('  report.append("")');
    func.push('  report.append("Ran %d test." % len(' + resultsVar + '))');
    func.push('  report.append("")');
    func.push('  if fails:');
    func.push('    report.append("FAILED (failures=%d)" % fails)');
    func.push('  else:');
    func.push('    report.append("OK")');
    func.push('  return "\\n".join(report)');
    func.push('');
    Blockly.Ruby.definitions_['unittest_report'] = func.join('\n');
  }
  // Setup global to hold test results.
  var code = resultsVar + ' = []\n';
  // Run tests (unindented).
  code += Blockly.Ruby.statementToCode(this, 'DO')
      .replace(/^  /, '').replace(/\n  /g, '\n');
  var reportVar = Blockly.Ruby.variableDB_.getDistinctName(
      'report', Blockly.Variables.NAME_TYPE);
  code += reportVar + ' = ' + Blockly.Ruby.unittest_main.report + '()\n';
  // Destroy results.
  code += resultsVar + ' = None\n';
  // Print the report.
  code += 'print(' + reportVar + ')\n';
  return code;
};

Blockly.Ruby.unittest_main.defineAssert_ = function() {
  if (!Blockly.Ruby.definitions_['unittest_assertequals']) {
    var resultsVar = Blockly.Ruby.variableDB_.getName('unittestResults',
        Blockly.Variables.NAME_TYPE);
    var functionName = Blockly.Ruby.variableDB_.getDistinctName(
        'assertEquals', Blockly.Generator.NAME_TYPE);
    Blockly.Ruby.unittest_main.assert_ = functionName;
    var func = [];
    func.push('def ' + functionName + '(actual, expected, message):');
    func.push('  # Asserts that a value equals another value.');
    func.push('  if ' + resultsVar + ' == None:');
    func.push('    raise Exception("Orphaned assert equals: " + message)');
    func.push('  if actual == expected:');
    func.push('    ' + resultsVar + '.append((True, "OK", message))');
    func.push('  else:');
    func.push('    ' + resultsVar + '.append((False, ' +
        '"Expected: %s\\nActual: %s" % (expected, actual), message))');
    func.push('');
    Blockly.Ruby.definitions_['unittest_assertequals'] = func.join('\n');
  }
  return Blockly.Ruby.unittest_main.assert_;
};

Blockly.Ruby.unittest_assertequals = function() {
  // Asserts that a value equals another value.
  var resultsVar = Blockly.Ruby.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  // Asserts that a value equals another value.
  var message = Blockly.Ruby.quote_(this.getTitleValue('MESSAGE'));
  var actual = Blockly.Ruby.valueToCode(this, 'ACTUAL',
      Blockly.Ruby.ORDER_NONE) || 'None';
  var expected = Blockly.Ruby.valueToCode(this, 'EXPECTED',
      Blockly.Ruby.ORDER_NONE) || 'None';
  return Blockly.Ruby.unittest_main.defineAssert_() +
      '(' + actual + ', ' + expected + ', ' + message + ')\n';
};

Blockly.Ruby.unittest_assertvalue = function() {
  // Asserts that a value is true, false, or null.
  var resultsVar = Blockly.Ruby.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  // Asserts that a value is true.
  var message = Blockly.Ruby.quote_(this.getTitleValue('MESSAGE'));
  var actual = Blockly.Ruby.valueToCode(this, 'ACTUAL',
      Blockly.Ruby.ORDER_NONE) || 'None';
  var expected = this.getTitleValue('EXPECTED');
  if (expected == 'TRUE') {
    expected = 'True';
  } else if (expected == 'FALSE') {
    expected = 'False';
  } else if (expected == 'NULL') {
    expected = 'None';
  }
  return Blockly.Ruby.unittest_main.defineAssert_() +
      '(' + actual + ', ' + expected + ', ' + message + ')\n';
};

Blockly.Ruby.unittest_fail = function() {
  // Always assert an error.
  var resultsVar = Blockly.Ruby.variableDB_.getName('unittestResults',
      Blockly.Variables.NAME_TYPE);
  var message = Blockly.Ruby.quote_(this.getTitleValue('MESSAGE'));
  if (!Blockly.Ruby.definitions_['unittest_fail']) {
    var functionName = Blockly.Ruby.variableDB_.getDistinctName(
        'fail', Blockly.Generator.NAME_TYPE);
    Blockly.Ruby.unittest_fail.assert = functionName;
    var func = [];
    func.push('def ' + functionName + '(message):');
    func.push('  # Always assert an error.');
    func.push('  if ' + resultsVar + ' == None:');
    func.push('    raise Exception("Orphaned assert equals: ' + message + '")');
    func.push('  ' + resultsVar + '.append((False, "Fail.", message))');
    func.push('');
    Blockly.Ruby.definitions_['unittest_fail'] = func.join('\n');
  }
  return Blockly.Ruby.unittest_fail.assert + '(' + message + ')\n';
};
